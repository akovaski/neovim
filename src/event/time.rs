...
#[no_mangle]
pub unsafe extern "C" fn time_watcher_close(watcher: &mut TimeWatcher, cb: time_cb) {
    watcher.close_cb = cb;
    uv_close(&mut watcher.uv, Some(close_cb));
}

unsafe extern "C" fn time_event(argv: *mut *mut libc::c_void) {
    let watcher: &mut TimeWatcher = (*argv).cast::<TimeWatcher>().as_mut().unwrap();
    watcher.cb.expect("non-null function pointer")(watcher, watcher.data);
}

unsafe extern "C" fn time_watcher_cb(handle: &mut uv_timer_t) {
    let watcher: &mut TimeWatcher = handle.data.cast::<TimeWatcher>().as_mut().unwrap();
    if watcher.blockable && !multiqueue_empty(watcher.events.as_ref().unwrap()) {
