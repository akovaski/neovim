...
pub unsafe extern "C" fn utf_char2cells(c: i32) -> i32 {
    if c >= 0x100 {
        //TODO: USE_WCHAR_FUNCTIONS
        if !utf_printable(c) {
            return 6; // unprintable, displays <xxxx>
        }
        if intable(doublewidth.as_ptr(), doublewidth.len(), c) {
            return 2;
        }
        if p_emoji != 0 && intable(emoji_width.as_ptr(), emoji_width.len(), c) {
            return 2;
        }
    ...
    if c >= 0x80 && *p_ambw == 'd' as u8 && intable(ambiguous.as_ptr(), ambiguous.len(), c) {
        return 2;
    }
...
pub unsafe extern "C" fn mb_string2cells_len(str: *const u8, size: size_t) -> size_t {
    ...
    for p in it!(p = str; *p != 0;
    p.offset(
        utf_ptr2len_len(p, size as i32 + p.offset_from(str) as i32) as isize,
    )) {
        clen += utf_ptr2cells(p) as usize;
    }
    return clen;
}
