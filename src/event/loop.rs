...
pub unsafe fn LOOP_PROCESS_EVENTS(
    ...
    match multiqueue {
        Some(multiqueue) if !multiqueue_empty(multiqueue) => {
...
pub unsafe extern "C" fn loop_init(mut loop_0: &mut Loop, _data: *mut libc::c_void) {
    ...
    loop_0.recursive = 0;
    loop_0.uv.data = loop_0 as *mut _ as *mut libc::c_void;
    loop_0.children = kl_init();
    loop_0.events = multiqueue_new_parent(Some(loop_on_put), loop_0 as *mut _ as *mut libc::c_void);
    loop_0.fast_events = multiqueue_new_child(loop_0.events.as_mut().unwrap());
...
pub unsafe extern "C" fn loop_poll_events(mut loop_0: &mut Loop, ms: libc::c_int) -> bool {
    ...
    if ms > 0 {
        *loop_0.poll_timer.data.cast() = false; // reset "timeout expired" flag

        // Dummy timer to ensure UV_RUN_ONCE does not block indefinitely for I/O.
        uv_timer_start(&mut loop_0.poll_timer, Some(timer_cb), ms as u64, ms as u64);
    } else if ms == 0 {
    ...
    if ms > 0 {
        timeout_expired = *loop_0.poll_timer.data.cast();
        uv_timer_stop(&mut loop_0.poll_timer);
    }
    loop_0.recursive -= 1; // Can re-enter uv_run now
    multiqueue_process_events(loop_0.fast_events.as_mut().unwrap());
...
pub unsafe extern "C" fn loop_schedule_fast(loop_0: &mut Loop, event: Event) {
    ...
    multiqueue_put_event(loop_0.thread_events.as_mut().unwrap(), event);
...
pub unsafe extern "C" fn loop_schedule_deferred(loop_0: &mut Loop, event: Event) {
    ...
    *eventp = event;
    ...
    loop_schedule_fast(
        loop_0,
        event_create(Some(loop_deferred_event), vargs!(loop_ptr, eventp)),
    );
}
...
pub unsafe extern "C" fn loop_purge(loop_0: &mut Loop) {
    uv_mutex_lock(&mut loop_0.mutex);
    multiqueue_purge_events(loop_0.thread_events.as_mut().unwrap());
    multiqueue_purge_events(loop_0.fast_events.as_mut().unwrap());
    uv_mutex_unlock(&mut loop_0.mutex);
}
...
pub unsafe extern "C" fn loop_size(loop_0: &mut Loop) -> libc::size_t {
    uv_mutex_lock(&mut loop_0.mutex);
    let rv: libc::size_t = multiqueue_size(loop_0.thread_events.as_ref().unwrap());
    uv_mutex_unlock(&mut loop_0.mutex);
    return rv;
}

unsafe extern "C" fn async_cb(handle: *mut uv_async_t) {
    let l: &mut Loop = (*(*handle).loop_0).data.cast::<Loop>().as_mut().unwrap();
    uv_mutex_lock(&mut l.mutex);
    // Flush thread_events to fast_events for processing on main loop.
    while !multiqueue_empty(l.thread_events.as_ref().unwrap()) {
        let ev: Event = multiqueue_get(l.thread_events.as_mut().unwrap());
        multiqueue_put_event(l.fast_events.as_mut().unwrap(), ev);
    }
